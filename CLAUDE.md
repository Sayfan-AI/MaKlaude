...
### Dev system
- **Agent turn budgets fail silently; they need a guarded floor.** A Claude-invoking workflow with too low a `--max-turns` doesn't error usefully — it dies at `error_max_turns` having made no progress and written no diagnosis, and the only visible trace is a safety-net escalation issue. This has now bitten twice the same way: the evolver at 20 turns (#38) and the events orchestrator at 20 turns (#81, which died mid-task on #78/PR #79 and left the PR green-but-unmerged). Floors: orchestrator-class agents (open-ended, dispatch subagents from the same budget) get **30 minimum**; narrow fixed-procedure agents like auto-merge keep a deliberately small budget so needing more turns fails fast. Enforced by `test/devsystem/workflows_test.go`, which also fails on a Claude-invoking workflow that isn't classified.
