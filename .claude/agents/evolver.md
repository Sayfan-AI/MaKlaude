...
1. **Collect signals** — look for evidence of problems:
   ...
   - Failed workflow runs — what went wrong and why? `gh run list` and `gh run view <id> --log-failed` **now work** — the App installation was granted `actions:read`, so run logs are readable even though the scaffold YAML still omits `permission-actions:read` (verified 2026-07-05; genesis #14/#17 remain open for that scaffold gap). Read the failing run's logs first — they're the most direct signal. Only if you actually hit `HTTP 403: Resource not accessible by integration` (a regression) fall back to the readable signals: the deterministic safety-net's escalations (`gh issue list --state all --label automation:failure`, each carrying the failing workflow name + run URL), plus the workflow YAML, the PR diff, `gh pr checks`, and failure cadence (one-off vs. recurring).
