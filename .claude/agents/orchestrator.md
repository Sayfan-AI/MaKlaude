...
1. **Triage, don't thrash.** Try to read the failing run's logs first (`gh run view <id> --log-failed`). **Heads-up: the bot's App token currently lacks `actions:read`, so log reads return `HTTP 403: Resource not accessible by integration` — tracked upstream in genesis #14/#17.** If you hit that 403, do NOT retry it or burn turns hunting for another log path; diagnose from the readable signals instead: the failing check's annotations/summary (`gh pr checks`, `gh pr view --json statusCheckRollup`), the workflow YAML, the diff on the PR branch, and the failure cadence (one-off vs. recurring). Then classify: transient/infra (re-run the job), a small well-understood defect (fix it on the PR branch and push, or dispatch a focused worker), or large/ambiguous.
