...
import "github.com/Sayfan-AI/MaKlaude/internal/remediate"
...
// Subject is what trust is asked about: a [Shape] and the [remediate.Fingerprint] of
// the specific fix being proposed for it.
//
// The two fields carry different halves of the question and are deliberately not
// merged. The shape is the operation class on a cluster — the unit a person REVOKES,
// the unit a failure poisons, and the unit the disclosure artifacts are marked with.
// The fingerprint is the fix — the unit a person APPROVED. Collapsing them into one
// key would make revocation as narrow as approval, so revoking "no more restarts on
// prod" would leave every fingerprint but the one named still firing, which is the
// opposite of what a person hitting that switch means.
//
// See [remediate.Proposal.Fingerprint] for what the fingerprint covers, and the
// package doc of [trust] for how the asymmetry is implemented: promotion is scoped to
// the fingerprint, demotion is scoped to the shape.
//
// A Subject with an empty fingerprint is never trusted. That is the reading of a
// proposal from a build that did not compute one, and it gates.
type Subject struct {
	// Shape is the operation class on a cluster.
	Shape Shape

	// Fingerprint identifies the fix. See [remediate.Proposal.Fingerprint].
	Fingerprint remediate.Fingerprint
}

// String renders the subject as "cluster/operation@fingerprint", a stable form for a
// log line or a test failure.
func (s Subject) String() string { return s.Shape.String() + "@" + string(s.Fingerprint) }

// TrustEvidence is a trust ledger's answer about one [Subject]: whether the fix has
...
// TrustOracle answers whether a [Subject] has earned autonomy. The trust ledger
...
// with the same subject against the same history return the same [TrustEvidence],
...
type TrustOracle interface {
	// Trust reports this subject's standing. It must not block, read a cluster, or
	// depend on when it is called.
	Trust(Subject) TrustEvidence
}

// StaticTrust is a fixed subject-to-citation map satisfying [TrustOracle]: a subject
// present in the map is trusted and its value is the citation, and a subject absent
...
// that computes every subject's standing up front can return one of these directly
// rather than implementing the interface itself.
//
// A nil map is a valid StaticTrust and trusts nothing.
type StaticTrust map[Subject]string

// Trust implements [TrustOracle].
func (s StaticTrust) Trust(subject Subject) TrustEvidence {
	citation, ok := s[subject]
